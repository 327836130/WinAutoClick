...
start "backend" cmd /k ^"cd /d ""%BACKEND%"" && ""%PY%"" -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload^"

echo [FRONTEND] starting npm run dev on 5173...
start "frontend" cmd /k ^"cd /d ""%FRONTEND%"" && (if not exist node_modules npm install) && npm run dev -- --host --port 5173^"
